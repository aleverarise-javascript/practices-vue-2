...
$db1 = new mysqli("localhost", "root", "lobo1415", "curso_vuejs_pago");

define('BD_SERVIDOR', 'localhost');
define('BD_NOMBRE', 'curso_vuejs_pago');
define('BD_USUARIO', 'root');
define('BD_PASSWORD', 'lobo1415');

$db = new PDO('mysql:host=' . BD_SERVIDOR . ';dbname=' . BD_NOMBRE . ';charset=utf8', BD_USUARIO, BD_PASSWORD);

$app->get("/restaurantes", function() use($db, $app) {
	
	$consulta = $db->prepare("SELECT * FROM restaurantes ORDER BY id DESC;");
    $consulta->execute();
...
$app->get("/restaurante/:id", function($id) use($db1, $app) {
	// sleep(3);
	$query = $db1->query("SELECT * FROM restaurantes WHERE id = $id;");
...
$app->post("/restaurantes", function() use($db, $app) {
    ...
    $consulta= $db->prepare("insert into restaurantes(nombre,direccion,descripcion,precio) values (:nombre,:direccion,:descripcion,:precio)");
